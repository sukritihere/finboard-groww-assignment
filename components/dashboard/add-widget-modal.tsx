...
export default function AddWidgetModal({ isOpen, onClose }: AddWidgetModalProps) {
  ...
  const handleTestAPI = async () => {
    ...
    if (result.success) {
      ...
      setTestMessage("API connection successful! 1 top-level fields found.")
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-slate-800 rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto border border-slate-700">
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            <label className="block text-white font-semibold mb-3">Widget Type</label>
          ...
          {/* Widget Title */}
          <div>
            <label className="block text-white font-semibold mb-2">Widget Title</label>
            <input
              type="text"
              value={title}
          ...
          {widgetType && (
            <div>
              <label className="block text-white font-semibold mb-2">API Presets</label>
              <select
                onChange={(e) => {
          ...
          {/* API URL */}
          <div>
            <label className="block text-white font-semibold mb-2">API URL</label>
          ...
          {/* Refresh Interval */}
          <div>
            <label className="block text-white font-semibold mb-2">Refresh Interval (seconds)</label>
            <input
              type="number"
          ...
          {widgetType === "chart" && availableFields.length > 0 && (
            <div className="space-y-4 p-4 bg-slate-700/50 rounded-lg border border-slate-600">
              <h3 className="text-white font-semibold">Chart Configuration</h3>

              <div>
                <label className="block text-slate-300 text-sm mb-2">Chart Type</label>
                <select
                  value={chartType}
              ...
              <div>
                <label className="block text-slate-300 text-sm mb-2">X-Axis Field</label>
                <select
                  value={xAxisField}
              ...
              <div>
                <label className="block text-slate-300 text-sm mb-2">Y-Axis Field</label>
                <select
                  value={yAxisField}
              ...
              <div>
                <label className="block text-slate-300 text-sm mb-2">Chart Color</label>
                <input
                  type="color"
                  value={chartColor}
          ...
          {availableFields.length > 0 && (
            <div className="space-y-4 p-4 bg-slate-700/50 rounded-lg border border-slate-600">
              <h3 className="text-white font-semibold">Select Fields to Display</h3>
                ...
                {availableFields.map((field) => (
                  <label key={field} className="flex items-center gap-3 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 rounded border-slate-500 bg-slate-600 text-emerald-600 cursor-pointer"
                    />
